...
uses
  ...
  ConnectorUnit, ConnectingUnit, ContinueUnit, GameLogUnit, ModalForm;
...
begin
  ...
  m_Dialogs := TDialogs.Create(ChessBoard);
//  m_GameLogForm := TGameLogForm.Create(ChessBoard);
  m_GameLogForm := m_Dialogs.CreateDialog(TGameLogForm, FALSE, FALSE) as TGameLogForm;
...
{$ENDIF}
        if ((ChessBoard.PositionColor = Black) or (not move_done)) then
          m_GameLogForm.AddMove(ChessBoard.NMoveDone, White, PString(d1)^)
        else
          m_GameLogForm.AddMove(ChessBoard.NMoveDone, Black, PString(d1)^);        
...
{$ENDIF}
                    if ((ChessBoard.PositionColor = Black) or (not move_done)) then
                      m_GameLogForm.AddMove(ChessBoard.NMoveDone, White, sl)
                    else
                      m_GameLogForm.AddMove(ChessBoard.NMoveDone, Black, sl);
                    move_done := TRUE;
                    TakebackGame.Enabled := TRUE;
                  end;
              end; {  with ChessBoard}
...
begin
  ...
  with lookFeelOptionsForm, ChessBoard do
    try
      ...
      GameLogBox.Checked := m_GameLogForm.Visible;
      ...
      m_GameLogForm.Visible := GameLogBox.Checked;
...
begin
  ...
  try
    ...
    m_GameLogForm.Visible := iniFile.ReadBool(PRIVATE_SECTION_NAME, GAME_LOG_KEY_NAME, TRUE);
  finally
    iniFile.Free;
  end;
end;
...
begin
  ...
  try
    ...
    iniFile.WriteBool(PRIVATE_SECTION_NAME, GAME_LOG_KEY_NAME, m_GameLogForm.Visible);
  finally
    iniFile.Free;
  end;
end;
