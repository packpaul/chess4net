...
begin
  ...
  // конвертирование системных символов
  Result := StringReplace(Result, '&gt;',   '>',  [rfReplaceAll]);
  Result := StringReplace(Result, '&amp;',  '&',  [rfReplaceAll]);
//  Result := StringReplace(Result, '&quot;', '"',  [rfReplaceAll]);
//  Result := StringReplace(Result, '&apos;', '''', [rfReplaceAll]);
//  Result := StringReplace(Result, '&lt;',   '<',  [rfReplaceAll]);
  // Здесь можно добавить другие символы из RFC, если нужно
end;
...
begin
  ...
  if not ((string(pMessage.name) = gpContactlistEntry.real_name) and
    ...
    exit; // Сообщение от др. контакта / др. вида

  if (string(pMessage._type) = INCOMING_PRIVATE_MESSAGE) then
    begin
      if FilterMsg(messageText) then
        begin
          // TODO: подавить сообщение
          // Trillian не имеет вомзожности подавлять сообщения. Возможно ли использовать ASM для перезаписи callstack(а)?
          // На текущий момент текст сообщения и имя оставляются пустые
          pMessage.text^ := #0;
